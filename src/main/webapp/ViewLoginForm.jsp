...
<form action="LoginController" class="form" method="POST">
    ...
    <input class="w3-input w3-border w3-light-grey" type="text" name="name" placeholder="Username" value="${user.name}" required pattern="^.{1,50}$" autocomplete="username"><br>
    <label for="pwd1" class="w3-text-black"><b>Password:</b></label><br>
  	<input class="w3-input w3-border w3-light-grey" type="password" id="pwd1" name="pwd" placeholder="Password" value="${user.pwd}" required pattern="^(?=.*[A-Z])(?=.*[!@#$&*\-])(?=.{8,})\S+$" autocomplete="current-password"><br><br>
