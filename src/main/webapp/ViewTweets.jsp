...
<c:forEach var="t" items="${tweets}"> 
 <div id="${t.id}" class="w3-container w3-card w3-section w3-white w3-round w3-animate-opacity"><br>
   ...
   <c:if test="${user != null}">
     <c:if test="${!t.isLiked}">
       <button id="button_${t.id}" type="button" class="likeTweet w3-button w3-light-grey w3-hover-red w3-margin-bottom">
         <i class="fa fa-heart"></i> &nbsp;Like
       </button>
     </c:if>
     <c:if test="${t.isLiked}">
       <button id="button_${t.id}" type="button" class="likeTweet w3-button w3-theme w3-hover-light-grey w3-margin-bottom">
         <i class="fa fa-heart"></i> &nbsp;Like
       </button>
     </c:if>
   </c:if>
   ...
   <c:if test="${user == null}">
     <button disabled type="button" class="likeTweet w3-button w3-theme w3-margin-bottom">
       <i class="fa fa-heart"></i> &nbsp;Like
     </button>
   </c:if>
   
   <c:if test="${user != null && user.name == t.uname || user.isAdmin}">
     <button type="button" class="delTweet w3-button w3-red w3-margin-bottom">
       <i class="fa fa-trash"></i> &nbsp;Delete
     </button> 
     <button type="button" class="editTweet w3-button w3-red w3-margin-bottom">
   ...
   <c:if test="${user != null}">
    ...
    <input id="add_comment_${t.id}"class="w3-input w3-border w3-light-grey w3-margin-bottom" type="text" name="content" placeholder="Add a comment...">
    <button class="w3-button w3-red w3-margin-bottom addComment" type="submit">Submit</button>
	</c:if>
